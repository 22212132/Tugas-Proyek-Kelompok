...
<body class="bg-blue-800 flex min-h-screen">
    ...
    <div class="w-1/6 bg-blue-800 text-white flex flex-col items-start py-8 px-6 space-y-4">
        <a href="{{ route('home') }}" 
            class="inline-flex items-center text-white mb-6 hover:text-indigo-200 transition-colors">
        ...
        <a href="{{ route('saldo') }}"
           class="flex items-center w-full space-x-2 {{ request()->routeIs('saldo') ? 'bg-white/10' : 'hover:bg-white/10' }} rounded-md px-3 py-2 transition">
            <i class="fa fa-wallet text-white"></i>
            <span>Saldo</span>
        </a>

        <a  href="{{ route('history') }}"
           class="flex items-center w-full space-x-2 {{ request()->routeIs('history') ? 'bg-white/10' : 'hover:bg-white/10' }} rounded-md px-3 py-2 transition">
